		...
		for (int i = 0; i < chiffres.size(); i++) {
			for (int j = i + 1; j < chiffres.size(); j++) {
				for (int o = 0; o < 6; o++) {
					...
					if ((o == 0) && (c1 != 1) && (c2 != 1)) {
						if (c1 * c2 == but) {
							System.out.println("Gagné " + c1 + "*" + c2 + "=" + new Integer(c1 * c2));
							return true;
						}
						newChiffres.add(c1 * c2);
						if(new Compte(newChiffres, but).estResolu()){
							System.out.println(c1 + "*" + c2 + "=" + new Integer(c1 * c2));
							return true;
						}
						...
						if (c2 - c1 == but) {
							System.out.println("Gagné " + c2 + "-" + c1 + "=" + new Integer(c2 - c1));
							return true;
						}
						newChiffres.add(c2 - c1);
						if(new Compte(newChiffres, but).estResolu()){
							System.out.println(c2 + "-" + c1 + "=" + new Integer(c2 - c1));
							return true;
						}
